...
function initagriculture(m::Model)
    ...
    agriculture[:othercropsirrigation] = convert(Vector,(knownareas[:total] - knownareas[:known]) ./ knownareas[:total])* recorded
...
function initagriculture(m::Model)
    ...
    recorded = readtable(datapath("extraction/USGS-2010.csv"))
    othercropirrigation = ((knownareas[:total] - knownareas[:known]) ./ knownareas[:total]) * config["timestep"] .* recorded[:, :IR_To] * 1382592. / (1000. * 12)
...
>>>>>>> aceda6b5908d3dbcf85171cfc0e84a6ecbbcb0b6
