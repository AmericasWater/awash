...
function values_waterdemand_recordedirrigation(m::Model, includegw::Bool, demandmodel::Model=nothing)
...
function values_waterdemand_recordeddomestic(m::Model, includegw::Bool, demandmodel::Model=nothing)
    if demandmodel == nothing
        if includegw
...
function values_waterdemand_recordedindustrial(m::Model, includegw::Bool, demandmodel::Model=nothing)
    if demandmodel == nothing
        if includegw
...
function values_waterdemand_recordedthermoelectric(m::Model, includegw::Bool, demandmodel::Model=nothing)
    if demandmodel == nothing
        if includegw
...
function values_waterdemand_recordedlivestock(m::Model, includegw::Bool, demandmodel::Model=nothing)
    if demandmodel == nothing
        if includegw
...
function values_waterdemand_recordedsurfacelivestock(m::Model)
    recorded = readtable(datapath("extraction/USGS-2010.csv"))
    gen(rr, tt) = config["timestep"] * recorded[rr, :LI_SW] * 1383. / 12.
