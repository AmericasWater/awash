...
@defcomp Agriculture begin
    ...
    irriarea=Variable(index=[regions, time], unit="Ha")
...
function run_timestep(s::Agriculture, tt::Int)
    ...
    for rr in d.regions
        ...
        for cc in d.crops
            ...
            #irriarea=v.totalirrigated[rr,tt]
...
function grad_agriculture_irriarea_irrigatedareascst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for tt in 1:numsteps
                for cc in 1:numcrops
    ...
    roomintersect(m, :Agriculture, :irriarea, :irrigatedareascst, generate)
end
...
function constraintoffset_agriculture_irriarea(m::Model)
    ...
    hallsingle(m, :Agriculture, :irriarea, (rr, tt) -> recorded[rr,tt])
end
