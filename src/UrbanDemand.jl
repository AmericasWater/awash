...
function run_timestep(c::UrbanDemand, tt::Int)
    ...
    for rr in d.regions
        ...
        #v.urbandemand[rr,tt]=p.data_demand[rr, tt]
    end
end
...
function initurbandemand(m::Model)
    ...
    #Replace domestic demand if there is regression NY urban model 
    urban=zeros(numcounties, numsteps);
    for rr in 1:numcounties
        for tt in 1:numsteps
            urban[rr,tt]=data_demand[rr,tt]
        end
    end
    urbandemand[:domesticdemand]=urban
...
function constraintoffset_urbandemand_waterdemand(m::Model)
    gen(rr, tt) = #m.parameters[:commercialdemand].values[rr, tt] + 
    m.parameters[:domesticdemand].values[rr,tt]
