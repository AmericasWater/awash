...
function run_timestep(c::WaterCost, tt::Int)
    ...
    for pp in 1:nrow(draws)
	    ...
	    if rr > 0
		    v.swcost[rr,tt] += p.swwithdrawals[pp,tt]*(p.unitswextractioncost[pp,tt]+p.unitdistributioncost[rr,tt])
	    end
    end

    for rr in 1:numcounties
	    aa = rr
	    v.gwcost[rr,tt] = (p.unitgwextractioncost[aa,tt] + p.unitdistributioncost[rr,tt]) * p.gwextraction[rr,tt]
...
function grad_watercost_costgw(m::Model)
	roomdiagonal(m, :WaterCost, :gwcost, :gwextraction, (rr, tt) -> m.parameters[:unitgwextractioncost].values[rr,tt] + m.parameters[:unitdistributioncost].values[rr,tt])
end
...
function grad_watercost_costswwithdrawals(m::Model)
     function generate(A, tt)
         ...
         for pp in 1:nrow(draws)
             ...
             rr = findfirst(mastercounties[:fips] .== fips)
             if rr > 0
		     A[rr, pp] = m.parameters[:unitswextractioncost].values[pp,tt] + m.parameters[:unitdistributioncost].values[rr,tt]
             end
         end
     end
