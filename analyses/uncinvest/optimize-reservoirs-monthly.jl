...
for envflowreq in [.37, .50]
    ...
    captures = reshape(sol.sol[parends[3]+1:parends[4]], numreservoirs, numscenarios, numsteps)[:, :, 13:24]
    ...
    gwextracts_nores = reshape(sol_nores.sol[parends_nores[3]+1:parends_nores[4]], numregions, numscenarios, numsteps)[:, :, 13:24]
    ...
    writetable("groundwater-monthly-$(envflowreq*100).csv", DataFrame(fips=masterregions[:fips], maxsums=vec(gwmaxs),
