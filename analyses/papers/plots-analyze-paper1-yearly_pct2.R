...
namescenario <- c("w/o can. w/o res.","w/ can. w/o res.","w/ can. w/ res.")
...
#  df_gw <- melt(apply(result[3,,,], c(2,1,3), sum))
  ...
  df_new <- cbind(df_d, df_sw$value, df_f$value)
  names(df_new) <- c("year","fips","scenario","dem", "SW", "Failure")
...
names(df_reg) <- c("year","fips","scenario","dem", "SW", "Failure","region")
...
h <- h+geom_boxplot(aes(fill=scenario)) + facet_grid(variable ~ region, scales = "free")#, margins = F))
if(screenplots){print(h)}
if(printplots){savePlot(h, file=paste0(savingresultspath, "bxplot_reg"), "wide")}

h <- ggplot(df_reg_pct, aes(scenario, percent))
h <- h+geom_boxplot(aes(fill=scenario)) + facet_grid(variable ~ region, scales = "free")
if(screenplots){print(h)}
...
p <- ggplot(data=subset(df_reg_vol,region %in% c("West" , "Northwest")),aes(x=year, y=volume)) +
  stat_summary(fun.y=mean, geom="line",  aes(colour=scenario, linetype = scenario)) #+
p <- p + facet_grid(variable ~ region, scales = "free")
if(screenplots){print(p)}
if(printplots){savePlot(p, file=paste0(savingresultspath, "time_reg_zoom"), "wide")}
...
names(dfst) <- c("year","fips","scenario","SW", "Failure","precip","region")

dfsty <- aggregate(.~year+scenario, sum, data=dfst[,c(1,3:6)])
p <- ggplot(data=dfsty,aes(x=SW, y=Failure)) +
  geom_point(aes(colour=precip)) +
  scale_color_gradient2(midpoint=0, high="blue", mid="yellow", low="red", space ="Lab" )
p <- p + facet_wrap(~ scenario, scales = "free")
if(screenplots){print(p)}
if(printplots){savePlot(p, file=paste0(savingresultspath, "sw-f-precip_nat"))}
...
for(ii in 1:length(levels(dfst$region))){
  ...
  p <- ggplot(data=dfsty,aes(x=SW, y=Failure)) + geom_point(aes(colour=precip))  +
    scale_color_gradient2(midpoint=0, high="blue", mid="yellow", low="red", space ="Lab")

  p <- p + facet_wrap(~ scenario, scales = "free") +
  labs(title=levels(dfst$region)[ii])
  if(printplots){savePlot(p, file=paste0(savingresultspath, "sw-f-precip_reg",levels(dfst$region)[ii],""))}
}
