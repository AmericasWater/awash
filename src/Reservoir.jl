...
function grad_reservoir_storage_captures(m::Model)
    roomsingle(m, :Reservoir, :storage, :captures, (vrr, vtt, prr, ptt) -> (1-m.external_parameters[:evaporation].values[prr])^(vtt-ptt) * ((vrr == prr) && (vtt >= ptt)))
end
...
function constraintoffset_reservoir_storage0(m::Model)
    gen(rr, tt) = (1-m.external_parameters[:evaporation].values[rr])^(tt-1) * m.external_parameters[:storage0].values[rr]
    hallsingle(m, :Reservoir, :storage, gen)
end
