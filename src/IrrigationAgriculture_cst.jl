...
@defcomp IrrigationAgriculture begin
    ...
    irrigatedareas_cst = Parameter(index=[regions, irrcrops], unit="Ha")
    rainfedareas_cst = Parameter(index=[regions, irrcrops], unit="Ha")
    ...
    totalareas_cst = Variable(index=[regions, irrcrops, time], unit="Ha")
...
function run_timestep(s::IrrigationAgriculture, tt::Int)
    ...
    for rr in d.regions
        ...
        for cc in d.irrcrops
            ...
            v.totalareas_cst[rr,cc,tt]=p.irrigatedareas_cst[rr,cc]+p.rainfedareas_cst[rr,cc]
            #allagarea += v.totalareas[rr, cc, tt]
            allagarea += v.totalareas_cst[rr,cc,tt]
...
function initirrigationagriculture(m::Model)
    ...
    if isempty(irrcrops)
        ...
        agriculture[:rainfedareas_cst] = zeros(Float64, (nrow(rainfeds), 0))
        agriculture[:irrigatedareas_cst] = zeros(Float64, (nrow(rainfeds), 0))
    else
        ...
        agriculture[:rainfedareas_cst] = convert(Matrix, rainfeds[:, columns])
        agriculture[:irrigatedareas_cst] = convert(Matrix, irrigateds[:, columns])
    end
...
function grad_irrigationagriculture_production_irrigatedareas_cst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for cc in 1:numirrcrops
                for tt in 1:numsteps
                    A[fromindex([rr,cc,tt],[numcounties,numirrcrops,numsteps]), fromindex([rr,cc],[numcounties,numirrcrops])] = exp(m.parameters[:logirrigatedyield].values[rr, cc, tt]) * 2.47105 * .99 * config["timestep"]/12
                end
            end
        end
        return A
    end
    roomintersect(m,:IrrigationAgriculture, :production, :rainfedareas_cst,generate)
end
...
function grad_irrigationagriculture_production_rainfedareas_cst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for cc in 1:numirrcrops
                for tt in 1:numsteps
                    A[fromindex([rr,cc,tt],[numcounties,numirrcrops,numsteps]), fromindex([rr,cc],[numcounties,numirrcrops])] = exp(m.parameters[:logirrigatedyield].values[rr, cc, tt] + m.parameters[:deficit_coeff].values[rr, cc] * max(0., m.parameters[:water_demand].values[cc] - m.parameters[:precipitation].values[rr, tt])) * 2.47105 * config["timestep"]/12 # Convert Ha to acres
                end
            end
        end
        return A
...
function grad_irrigationagriculture_totalirrigation_irrigatedareas_cst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for tt in 1:numsteps
                for cc in 1:numirrcrops
                    A[fromindex([rr, tt], [numcounties, numsteps]),fromindex([rr, cc], [numcounties, numirrcrops])] = max(0., m.parameters[:water_demand].values[cc] - m.parameters[:precipitation].values[rr, tt]) / 100 
                end
            end
        end
    ...
    roomintersect(m, :IrrigationAgriculture, :totalirrigation, :irrigatedareas_cst, generate)
end
...
function grad_irrigationagriculture_allagarea_irrigatedareas_cst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for tt in 1:numsteps
                for cc in 1:numirrcrops
                    A[fromindex([rr, tt], [numcounties, numsteps]),fromindex([rr, cc], [numcounties, numirrcrops])] = 1.
                end
            end
        end
        return A
    end
    roomintersect(m, :IrrigationAgriculture, :allagarea, :irrigatedareas_cst, generate)
end
...
function grad_irrigationagriculture_allagarea_rainfedareas_cst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for tt in 1:numsteps
                for cc in 1:numirrcrops
    ...
    roomintersect(m, :IrrigationAgriculture, :allagarea, :rainfedareas_cst, generate)
end
...
function grad_irrigationagriculture_cost_rainfedareas_cst(m::Model)
        function generate(A)
        for rr in 1:numcounties
            for cc in 1:numirrcrops
                for tt in 1:numsteps
                    A[fromindex([rr,cc,tt],[numcounties,numirrcrops,numsteps]), fromindex([rr,cc],[numcounties,numirrcrops])] = cultivation_costs[irrcrops[cc]] * 2.47105 * config["timestep"]/12
                end
            end
        end
        return A
    end
    roomintersect(m,:IrrigationAgriculture, :irrcultivationcost, :rainfedareas_cst,generate)
end
...
function grad_irrigationagriculture_cost_irrigatedareas_cst(m::Model)
        function generate(A)
        for rr in 1:numcounties
            for cc in 1:numirrcrops
                for tt in 1:numsteps
    ...
    roomintersect(m,:IrrigationAgriculture, :irrcultivationcost, :irrigatedareas_cst,generate)
end
