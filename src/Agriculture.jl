...
function grad_agriculture_production_irrigatedareascst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for cc in 1:numcrops
                for tt in 1:numsteps
                    A[fromindex([rr,cc,tt],[numcounties,numcrops,numsteps]), fromindex([rr,cc],[numcounties,numcrops])] = exp(m.parameters[:logirrigatedyield].values[rr, cc, tt]) * 2.47105 * .99* config["timestep"]/12
                end
            end
        end
        return A
...
function grad_agriculture_production_rainfedareascst(m::Model)
    function generate(A)
        for rr in 1:numcounties
            for cc in 1:numcrops
                for tt in 1:numsteps
                    A[fromindex([rr,cc,tt],[numcounties,numcrops,numsteps]), fromindex([rr,cc],[numcounties,numcrops])] = exp(m.parameters[:logirrigatedyield].values[rr, cc, tt] + m.parameters[:deficit_coeff].values[rr, cc] * max(0., m.parameters[:water_demand].values[cc] - m.parameters[:precipitation].values[rr, tt])) * 2.47105 * config["timestep"]/12
                end
            end
        end
        return A
