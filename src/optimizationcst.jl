...
parameters = [:waterfromsupersource, :waterfromgw, :withdrawals, :rainfedareascst, :irrigatedareascst]
constcomps = [:Agriculture, :WaterNetwork, :Allocation]
constraints = [:allagarea, :outflows, :balance]
...
if redohouse
    ...
    setobjective!(house, -varsum(grad_agriculture_cost_rainfedareascst(m))) #rr,cc,tt
    setobjective!(house, -varsum(grad_agriculture_cost_irrigatedareascst(m))) #rr,cc,tt
    setobjective!(house, -varsum(grad_allocation_cost_waterfromsupersource(m)))
    setobjective!(house, deriv_market_totalrevenue_produced(m) * room_relabel(grad_agriculture_production_rainfedareascst(m), :production, :Market, :produced))
    setobjective!(house, deriv_market_totalrevenue_produced(m) * room_relabel(grad_agriculture_production_irrigatedareascst(m), :production, :Market, :produced))
    #Objective 
    ...
    grad_agriculture_totalirrigation_irrigatedareascst(m)) # +
    ...
    setconstraint!(house, grad_agriculture_allagarea_irrigatedareascst(m)) # +
    setconstraint!(house, grad_agriculture_allagarea_rainfedareascst(m)) # +
