...
function grad_irrigationagriculture_production_irrigatedareas(m::Model)
    ## Common rr, cc, tt
    roomdiagonalintersect(m, :IrrigationAgriculture, :production, :irrigatedareas, (ss1) -> exp.(m.md.external_params[:IrrigationAgriculture_logirrigatedyield].values[:, :, ss1, :]) * 2.47105 * .99 * config["timestep"]/12) # Convert Ha to acres
...
function grad_irrigationagriculture_production_rainfedareas(m::Model)
    ...
    function gen(ss1)
        ...
        for cc in 1:numirrcrops
            for tt in 1:numsteps
                A[:, cc, tt] = exp.(m.md.external_params[:IrrigationAgriculture_logirrigatedyield].values[:, cc, ss1, tt] + m.md.external_params[:IrrigationAgriculture_deficit_coeff].values[:, cc] * max.(0., m.md.external_params[:IrrigationAgriculture_water_demand].values[cc] - m.md.external_params[:IrrigationAgriculture_precipitation].values[:, ss1, tt])) * 2.47105 * config["timestep"]/12 # Convert Ha to acres
            end
        end
        A
    end
        ...
        for rr in 1:numcounties
            for cc in 1:numirrcrops
                for ss in 1:numscenarios
                    A[fromindex([rr, ss], [numcounties, numscenarios]), fromindex([rr, cc, ss], [numcounties, numirrcrops, numscenarios])] = max(0., m.md.external_params[:IrrigationAgriculture_water_demand].values[cc] - m.md.external_params[:IrrigationAgriculture_precipitation].values[rr, ss, tt]) / 100
                end
            end
        end
