...
function deriv_market_totalrevenue_produced(m::Model)
    gen(rr, cc) = m.md.external_params[:Market_domestic_prices].values[rr, cc]
    hallsingle(m, :Market, :produced, gen, [:scenarios, :time])
end

function deriv_market_totalrevenue_internationalsales(m::Model)
    gen(rr, cc) = -m.md.external_params[:Marketdomestic_prices].values[rr, cc] + m.md.external_params[:Market_international_prices].values[rr, cc]
