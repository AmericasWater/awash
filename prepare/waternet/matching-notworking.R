...
canalshapes <- importShapefile("canals/canals", readDBF=T)
proj.abbr <- attr(canals, "projection")

## Find all USGS canals that lie upon an NHD canal
canals <- read.delim("canals/USGS_monthly_canals_database.txt", colClasses=c(rep("character", 3), rep("numeric", 3), "character", rep("numeric", 2)))
...
write.csv(extracts, "canalnetwork.csv", row.names=F)
