...
if config["timestep"] == 12
	configname = "$configname-yearly"
elseif config["timestep"] == 1
	configname = "$configname-monthly"
end


if config["dataset"] == "counties"
	config["startweather"] = (starty-1950)*12+1;
else 
	config["startweather"] = (starty-1451)*12+1;
end
for yy in starty:simyrs:(endy-simyrs+1)
    ...
    writecsv("$savingresultspath/failure-$configname-$yy.csv", reshape(sol.sol[1:varlens[1]], numregions, numsteps));
