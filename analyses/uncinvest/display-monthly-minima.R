...
totalflow <- as.vector(t(ncvar_get(ncin, "contributing_area")) %*% flows) # 1000 m^3
...
totalflow2 <- rep(0, length(totalflow))
for (ii in 1:nrow(df)) {
    print(ii)
    ...
    totalflow2[!is.nan(flow)] <- totalflow2[!is.nan(flow)] + flow[!is.nan(flow)]
...
plot(totalflow / 14.1821, totalflow2, xlim=c(0, max(totalflow / 14.1821, totalflow2)), ylim=c(0, max(totalflow / 14.1821, totalflow2)), xlab="Sum of totalflow * area / 14.1821 over all gauges (1000 m^3)", ylab="Sum of totalflow * area over all counties (1000 m^3)")
