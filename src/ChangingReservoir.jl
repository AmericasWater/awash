...
function grad_reservoir_storage_captures(m::Model)
    roomchunks(m, :Reservoir, :storage, :captures, (vss, vtt, pss, ptt) -> (vtt >= ptt && vss == pss) ? spdiagm((1-m.md.external_params[:Reservoir_evaporation].values[:, vss, vtt]).^(config["timestep"]*(vtt-ptt)), 0) : spzeros(numreservoirs, numreservoirs), [:scenarios, :time], [:scenarios, :time])
end
...
function constraintoffset_reservoir_storagecapacitymax0(m::Model)
    gen(rr) = m.md.external_params[:Reservoir_storagecapacitymax0].values[rr]
    hallsingle(m, :Reservoir, :storage, gen, [:scenarios, :time])
end
...
function constraintoffset_reservoir_storage0(m::Model)
    gen(rr, ss, tt) = (1-m.md.external_params[:Reservoir_evaporation].values[rr, ss, tt])^(tt*config["timestep"]) * m.md.external_params[:Reservoir_storage0].values[rr]
...
function grad_reservoir_investcost_storagecapacitymax(m::Model)
    roomdiagonal(m, :Reservoir, :investcost, :storagecapacitymax, m.md.external_params[:Reservoir_unitcostcapacity].value, [:scenarios])
end
...
function grad_reservoir_investcost_increasestorage(m::Model)
    ...
    costs = marginalcost.(m.md.external_params[:Reservoir_storagecapacitymax0].values)
